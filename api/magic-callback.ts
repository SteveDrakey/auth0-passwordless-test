...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!code) {
    ...
    return res.redirect(`/#magic-error?error=${encodeURIComponent(errorMsg)}`);
  }
  ...
  try {
    ...
    return res.redirect(`/#magic-complete?${fragment.toString()}`);
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
    return res.redirect(`/#magic-error?error=${encodeURIComponent(msg)}`);
  }
}
